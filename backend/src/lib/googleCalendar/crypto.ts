// AES-256-GCM encryption for Google OAuth tokens stored at rest in the database.
// Fails closed when the key is missing (matches the QuickBooks token-encryption convention).
//
...
function getKey(): Buffer {
  ...
  if (!raw) {
    throw new Error('GOOGLE_TOKEN_ENC_KEY is not set; refusing to read/write Google tokens')
  }
