#!/bin/bash
# JobDock Database Migration Setup Script
# Run this on the bastion host to automatically set up and run migrations
...
DB_HOST=$(echo $DB_SECRET | grep -o '"host":"[^"]*"' | cut -d'"' -f4)
...
# Download Prisma schema from GitHub (or you'll need to paste it)
...
echo "✅ Migration lock file created"
echo ""
...
# Run migrations using Prisma's built-in migration tool
...
echo "  npx prisma db execute --stdin <<< \"SELECT tablename FROM pg_tables WHERE schemaname = 'public';\""
echo ""
